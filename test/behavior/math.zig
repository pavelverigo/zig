...
fn expectMulWithOverflow(comptime T: type, a: T, b: T, mul: T, bit: u1) !void {
...
test "basic @mulWithOverflow" {
    ...
    try expectMulWithOverflow(u8, 86, 3, 2, 1);
    try expectMulWithOverflow(u8, 85, 3, 255, 0);

    try expectMulWithOverflow(u8, 123, 2, 246, 0);
    try expectMulWithOverflow(u8, 123, 4, 236, 1);
}
...
test "extensive @mulWithOverflow" {
    ...
    try expectMulWithOverflow(u5, 3, 10, 30, 0);
    try expectMulWithOverflow(u5, 3, 11, 1, 1);
    try expectMulWithOverflow(i5, 3, -5, -15, 0);
    try expectMulWithOverflow(i5, 3, -6, 14, 1);

    try expectMulWithOverflow(u8, 3, 85, 255, 0);
    try expectMulWithOverflow(u8, 3, 86, 2, 1);
    try expectMulWithOverflow(i8, 3, -42, -126, 0);
    try expectMulWithOverflow(i8, 3, -43, 127, 1);

    try expectMulWithOverflow(u14, 3, 0x1555, 0x3fff, 0);
    try expectMulWithOverflow(u14, 3, 0x1556, 2, 1);
    try expectMulWithOverflow(i14, 3, -0xaaa, -0x1ffe, 0);
    try expectMulWithOverflow(i14, 3, -0xaab, 0x1fff, 1);

    try expectMulWithOverflow(u16, 3, 0x5555, 0xffff, 0);
    try expectMulWithOverflow(u16, 3, 0x5556, 2, 1);
    try expectMulWithOverflow(i16, 3, -0x2aaa, -0x7ffe, 0);
    try expectMulWithOverflow(i16, 3, -0x2aab, 0x7fff, 1);

    try expectMulWithOverflow(u30, 3, 0x15555555, 0x3fffffff, 0);
    try expectMulWithOverflow(u30, 3, 0x15555556, 2, 1);
    try expectMulWithOverflow(i30, 3, -0xaaaaaaa, -0x1ffffffe, 0);
    try expectMulWithOverflow(i30, 3, -0xaaaaaab, 0x1fffffff, 1);

    try expectMulWithOverflow(u32, 3, 0x55555555, 0xffffffff, 0);
    try expectMulWithOverflow(u32, 3, 0x55555556, 2, 1);
    try expectMulWithOverflow(i32, 3, -0x2aaaaaaa, -0x7ffffffe, 0);
    try expectMulWithOverflow(i32, 3, -0x2aaaaaab, 0x7fffffff, 1);

    try expectMulWithOverflow(u31, 1 << 30, 1 << 30, 0, 1);
    try expectMulWithOverflow(i31, minInt(i31), minInt(i31), 0, 1);
}
...
test "@mulWithOverflow bitsize > 32" {
    ...
    try expectMulWithOverflow(u40, 3, 0x55_5555_5555, 0xff_ffff_ffff, 0);
    try expectMulWithOverflow(u40, 3, 0x55_5555_5556, 2, 1);
    try expectMulWithOverflow(u40, 0x10_0000_0000, 0x10_0000_0000, 0, 1);

    try expectMulWithOverflow(i40, 3, -0x2a_aaaa_aaaa, -0x7f_ffff_fffe, 0);
    try expectMulWithOverflow(i40, 3, -0x2a_aaaa_aaab, 0x7f_ffff_ffff, 1);
    try expectMulWithOverflow(i40, 6, -0x2a_aaaa_aaab, -2, 1);
    try expectMulWithOverflow(i40, 0x08_0000_0000, -0x08_0000_0001, -0x8_0000_0000, 1);

    try expectMulWithOverflow(u62, 3, 0x1555555555555555, 0x3fffffffffffffff, 0);
    try expectMulWithOverflow(u62, 3, 0x1555555555555556, 2, 1);
    try expectMulWithOverflow(i62, 3, -0xaaaaaaaaaaaaaaa, -0x1ffffffffffffffe, 0);
    try expectMulWithOverflow(i62, 3, -0xaaaaaaaaaaaaaab, 0x1fffffffffffffff, 1);

    try expectMulWithOverflow(u64, 3, 0x5555555555555555, 0xffffffffffffffff, 0);
    try expectMulWithOverflow(u64, 3, 0x5555555555555556, 2, 1);
    try expectMulWithOverflow(i64, 3, -0x2aaaaaaaaaaaaaaa, -0x7ffffffffffffffe, 0);
    try expectMulWithOverflow(i64, 3, -0x2aaaaaaaaaaaaaab, 0x7fffffffffffffff, 1);
}
...
fn expectShlWithOverflow(comptime T: type, a: T, b: math.Log2Int(T), shl: T, bit: u1) !void {
...
test "@shlWithOverflow" {
    ...
    try expectShlWithOverflow(u4, 2, 1, 4, 0);
    try expectShlWithOverflow(u4, 2, 3, 0, 1);

    try expectShlWithOverflow(i9, 127, 1, 254, 0);
    try expectShlWithOverflow(i9, 127, 2, -4, 1);

    try expectShlWithOverflow(u16, 0b0010111111111111, 3, 0b0111111111111000, 1);
    try expectShlWithOverflow(u16, 0b0010111111111111, 2, 0b1011111111111100, 0);

    try expectShlWithOverflow(u16, 0b0000_0000_0000_0011, 15, 0b1000_0000_0000_0000, 1);
    try expectShlWithOverflow(u16, 0b0000_0000_0000_0011, 14, 0b1100_0000_0000_0000, 0);
}
...
test "@shlWithOverflow > 64 bits" {
    ...
    try expectShlWithOverflow(u65, 0x0_0100_0000_0000_0000, 7, 0x0_8000_0000_0000_0000, 0);
    try expectShlWithOverflow(u65, 0x0_0100_0000_0000_0000, 8, 0x1_0000_0000_0000_0000, 0);
    try expectShlWithOverflow(u65, 0x0_0100_0000_0000_0000, 9, 0, 1);
    try expectShlWithOverflow(u65, 0x0_0100_0000_0000_0000, 10, 0, 1);

    try expectShlWithOverflow(u128, 0x0100_0000_0000_0000_0000000000000000, 6, 0x4000_0000_0000_0000_0000000000000000, 0);
    try expectShlWithOverflow(u128, 0x0100_0000_0000_0000_0000000000000000, 7, 0x8000_0000_0000_0000_0000000000000000, 0);
    try expectShlWithOverflow(u128, 0x0100_0000_0000_0000_0000000000000000, 8, 0, 1);
    try expectShlWithOverflow(u128, 0x0100_0000_0000_0000_0000000000000000, 9, 0, 1);

    try expectShlWithOverflow(i65, 0x0_0100_0000_0000_0000, 7, 0x0_8000_0000_0000_0000, 0);
    try expectShlWithOverflow(i65, 0x0_0100_0000_0000_0000, 8, minInt(i65), 1);
    try expectShlWithOverflow(i65, 0x0_0100_0000_0000_0000, 9, 0, 1);
    try expectShlWithOverflow(i65, 0x0_0100_0000_0000_0000, 10, 0, 1);

    try expectShlWithOverflow(i128, 0x0100_0000_0000_0000_0000000000000000, 6, 0x4000_0000_0000_0000_0000000000000000, 0);
    try expectShlWithOverflow(i128, 0x0100_0000_0000_0000_0000000000000000, 7, minInt(i128), 1);
    try expectShlWithOverflow(i128, 0x0100_0000_0000_0000_0000000000000000, 8, 0, 1);
    try expectShlWithOverflow(i128, 0x0100_0000_0000_0000_0000000000000000, 9, 0, 1);
}
