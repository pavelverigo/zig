...
fn resolveInst(func: *CodeGen, ref: Air.Inst.Ref) InnerError!WValue {
    ...
    const result: WValue = if (isByRef(ty, pt))
        .{ .memory = try func.bin_file.lowerUnnamedConst(pt, val, func.decl_index) }
    else
...
/// NOTE: if result == .stack, it will stored in .local
...
/// TODO: Merge this function with abi.classifyType, having different one for each callconv
...
fn airSliceElemVal(func: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const elem_result = if (isByRef(elem_ty, pt))
        .stack
    else
...
fn airCmpxchg(func: *CodeGen, inst: Air.Inst.Index) InnerError!void {
    ...
    const result = if (isByRef(result_ty, pt)) val: {
